...
# 
        ...
        audio_path = pathlib.Path(cfg.path.scenes_folder) / f"{scene}_mix_CH1.wav"
        audio, _ = load_audio(audio_path, sr=df_state.sr())
        # Denoise the audio
        enhanced = enhanceDFN(model, df_state, audio)
        ...
        out_l = amplify_signal(
            enhanced[:, 0], listener.audiogram_left, enhancer, compressor
        )
        out_r = amplify_signal(
            enhanced[:, 1], listener.audiogram_right, enhancer, compressor
        )
